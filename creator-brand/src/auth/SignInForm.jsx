...
export default function SignInForm() {
  ...
  // Bug fix: showPassword must NOT be in a child component — keep here to avoid re-mount on toggle
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              dismissAllToasts();
              setTimeout(() => {
                navigate(user.userType === 'influencer' ? '/influencer_dashboard' : '/org_dashboard');
              }, 300);
            }, 2000);
          }, 300);
        }, 1000);
  ...
  const handleForgotSubmit = async (e) => {
    ...
    try {
      ...
      if (data.dev_token) { setResetToken(data.dev_token); setTimeout(() => setView('reset'), 1500); }
  ...
  if (view === 'forgot-sent') return (
    <div className="w-full h-full flex items-center justify-center p-6">
      <div className="w-full max-w-sm space-y-5 text-center">
        ...
        <p className="text-gray-400 text-sm leading-relaxed">We sent a reset link to <span className="text-white">{forgotEmail}</span>. It expires in 1 hour.</p>
        <button onClick={() => setView('auth')} className="text-gray-400 hover:text-white text-sm transition-colors underline underline-offset-2">Back to login</button>
      </div>
    </div>
  );
      ...
      <div className="w-full max-w-sm space-y-5">
        ...
        <form onSubmit={handleResetSubmit} className="space-y-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex items-center bg-gray-950 border border-gray-800 rounded-lg px-3 py-2 focus-within:border-white transition">
              <Lock className="size-4 mr-2 text-gray-400 flex-shrink-0" />
              <input
                type={showNewPassword ? 'text' : 'password'}
                required value={newPassword} onChange={e => setNewPassword(e.target.value)} minLength={6}
                className="bg-transparent w-full outline-none text-white placeholder-gray-500 text-sm"
                placeholder="••••••••" />
              <button
                type="button"
                onMouseDown={e => { e.preventDefault(); setShowNewPassword(p => !p); }}
          ...
          {forgotMsg && <p className={`text-xs ${forgotMsg.includes('reset') ? 'text-green-400' : 'text-red-400'}`}>{forgotMsg}</p>}
          <button type="submit" disabled={forgotLoading}
            className="w-full bg-white text-black font-medium py-2 rounded-lg hover:bg-gray-200 transition-colors text-sm disabled:opacity-50 flex items-center justify-center gap-2">
            {forgotLoading && <Loader2 className="w-4 h-4 animate-spin" />}
            Reset password
    ...
    <div className="w-full h-full relative text-center flex flex-col sm:flex-row overflow-y-auto">
      ...
      <div className="relative hidden sm:block sm:w-1/2 text-white flex-shrink-0">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center bg-black/30">
          ...
          <p className="text-base text-gray-200 max-w-xs px-4">
      ...
      {/* Right: form */}
      <div className="w-full sm:w-1/2 p-5 sm:p-8 flex items-center justify-center overflow-y-auto">
        <div className="w-full max-w-sm space-y-4">
          ...
          <div className="flex border-b border-gray-800">
            {['login', 'register'].map(tab => (
              <button key={tab} type="button" onClick={() => handleTabChange(tab)}
                className={`flex-1 py-1.5 text-center font-medium text-sm transition-colors capitalize ${
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select value={userType} onChange={e => setUserType(e.target.value)}
              className="bg-gray-950 border border-gray-800 rounded-lg px-3 py-2 text-white text-sm focus:border-white outline-none transition">
          ...
          {activeTab === 'register' && (
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex items-center bg-gray-950 border border-gray-800 rounded-lg px-3 py-2 focus-within:border-white transition">
                <UserPlus className="size-4 mr-2 text-gray-400 flex-shrink-0" />
          ...
          {/* Password — INLINED, not a child component, to prevent re-mount on toggle */}
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex items-center bg-gray-950 border border-gray-800 rounded-lg px-3 py-2 focus-within:border-white transition">
              ...
              />
              {/* onMouseDown + preventDefault keeps focus on the input */}
              <button
                type="button"
              ...
              >
                {/* Eye OPEN when visible, Eye CLOSED when hidden */}
          ...
          <button
            ...
            className="w-full bg-white text-black font-medium py-2.5 rounded-lg hover:bg-gray-200 transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
